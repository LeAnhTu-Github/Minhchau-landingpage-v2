...
import { Mail, Phone, MapPin, MessageSquare, Clock, ShieldCheck, FileCheck } from "lucide-react";
import { Container } from "@/components/ui/container";
import { ContactForm } from "@/components/sections/contact-form";
import { FadeInView, StaggerContainer, ScaleInView } from "@/components/ui/animations";

export const metadata: Metadata = {
  title: "Liên hệ",
  description: "Liên hệ với Minh Châu để nhận tư vấn và báo giá chi tiết cho các giải pháp CNTT, dịch vụ viễn thông và bảo trì SLA.",
};
              ...
              <StaggerContainer className="flex flex-col gap-12">
                ...
                <ScaleInView delay={0.6} className="flex flex-col gap-8 p-12 bg-linear-to-br from-cyan-600 to-blue-700 rounded-[50px] text-white shadow-3xl relative overflow-hidden group">
                  ...
                  <ul className="space-y-6 relative z-10">
                    {[
                      { icon: Clock, text: "Phản hồi trong vòng 4-8 giờ làm việc" },
                      { icon: ShieldCheck, text: "Sẵn sàng ký kết bảo mật (NDA)" },
                      { icon: FileCheck, text: "Lộ trình nghiệm thu rõ ràng (Milestones)" }
                    ].map((item, i) => (
                      <li key={i} className="flex gap-4 items-center group/item">
                        ...
                        <span className="font-bold text-lg text-cyan-50">{item.text}</span>
                      </li>
                    ))}
                  </ul>
                </ScaleInView>
      ...
      <section className="py-24 bg-slate-900/30">
        <Container>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { q: "Thời gian phản hồi là bao lâu?", a: "Chúng tôi cam kết phản hồi các yêu cầu tư vấn trong vòng 4-8 giờ làm việc." },
              { q: "Có ký bảo mật (NDA) không?", a: "Có. Minh Châu sẵn sàng ký NDA trước khi trao đổi sâu về nghiệp vụ." },
              { q: "Triển khai theo hình thức nào?", a: "Trọn gói (Fixed-price), Thuê đội (Dedicated team) hoặc vận hành theo SLA." },
              { q: "Có hỗ trợ nghiệm thu không?", a: "Có. Chúng tôi cung cấp đầy đủ Test plan, Test case và báo cáo QA chuyên nghiệp." },

        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link
                href="/contact"
                className="px-8 py-4 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-xl shadow-cyan-900/30 flex items-center justify-center gap-2 transition-all hover:scale-[1.02]"
              >
                Nhận tư vấn & báo giá
              ...
              <Link
                href="/projects"
                className="px-8 py-4 bg-slate-900 border border-slate-800 hover:bg-slate-800 text-white font-bold rounded-xl transition-all hover:scale-[1.02] flex items-center justify-center"
              >
                Xem dự án tiêu biểu
              </Link>
            </div>
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="relative z-10 rounded-2xl overflow-hidden shadow-2xl shadow-cyan-500/10 border border-slate-800">
              <ImageWithFallback
              ...
              <div className="absolute bottom-6 left-6 right-6 p-4 rounded-xl bg-slate-900/60 backdrop-blur-md border border-slate-700 flex items-center justify-between">
                <div>
                  <p className="text-white font-bold text-lg">Đội ngũ chuyên nghiệp</p>
                  <p className="text-slate-400 text-sm">86% trình độ Đại học trở lên</p>
                </div>
                <div className="flex -space-x-2">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="w-10 h-10 rounded-full border-2 border-slate-800 bg-slate-700 overflow-hidden relative">
                      <ImageWithFallback src={`https://i.pravatar.cc/150?u=${i}`} alt="Team Member" width={40} height={40} />
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Decorative elements */}
            <div className="absolute -top-6 -right-6 w-32 h-32 bg-cyan-500/20 rounded-full blur-3xl" />
